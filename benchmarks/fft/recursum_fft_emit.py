...
def liveness_slots(topo: List[Node], roots: List[Node], reuse: bool = True):
    ...
    `reuse=False` disables the recycling and gives every intermediate its own
    slot -- the naive baseline. It exists for the same reason it does in
    recursum_dag_emit.liveness_slots: the only way to say what the
    optimization is worth on a given target is to build both and measure. The
    two allocators now take the same flag so an ON/OFF experiment can be run
    against either emitter without special-casing which one it is."""
...
def emit_codelet(R: int, sign: int, name: str,
                 reuse: bool = True) -> Tuple[str, int, int]:
    reset_pool()
    ...
    slot, peak = liveness_slots(topo, roots, reuse=reuse)
...
def generate(radices=(2, 4, 8), prefix="scdt_fft_dft", reuse=True):
    ...
    for R in radices:
        for sign, tag in ((-1, "fwd"), (+1, "inv")):
            ...
            code, nops, peak = emit_codelet(R, sign, name, reuse=reuse)
            hdr.append(code)
            _ = code
