...
// erf/upward branch). Same robust routine as the J/K driver. The nuclear path
// calls g_boys (below), so a timing harness can swap in a shared fast Boys
// (libint2's Chebyshev) for a fair comparison, exactly as the ERI benchmark did.
...
// Boys evaluator the nuclear path uses; default = accurate routine above.
// A benchmark can point this at libint2's shared Chebyshev for a fair timing.
using boys_fn = void(*)(int,double,double*);
inline boys_fn g_boys = &boys;
      ...
      for(size_t jp=0;jp<G.e.size();++jp){ double be=G.e[jp], wG=G.d[jp]*G.N[jp];
        ...
        if(op==OVLP){ double S00=K*std::pow(M_PI/zeta,1.5);
            ...
            for(size_t k=0;k<Z.size();++k){
                ...
                double F0[32]; g_boys(maxm,Tn,F0);
